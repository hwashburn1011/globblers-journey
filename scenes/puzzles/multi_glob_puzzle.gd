...
func _create_visual() -> void:
	...
	# Step indicators — little green/dark boxes showing progress
	...
	if results.size() >= needed:
		...
		if _current_step < _step_indicators.size():
			...
			if ind.material_override:
				ind.material_override.emission = Color(0.224, 1.0, 0.078)
				ind.material_override.emission_energy_multiplier = 2.0
				ind.material_override.albedo_color = Color(0.1, 0.5, 0.05)
...
func _on_reset() -> void:
	...
	for ind in _step_indicators:
		if ind.material_override:
			ind.material_override.emission = Color(0.05, 0.15, 0.05)
