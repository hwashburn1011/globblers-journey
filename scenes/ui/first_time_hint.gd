...
# V2.0 restyle: terminal-monospace font, scanline shader background, green border frame.
...
func _build_ui(title: String, body: String) -> void:
	...
	# Terminal-style panel styling with double-thick green border
	...
	# Scanline shader overlay on the panel background
	var shader_mat := ShaderMaterial.new()
	shader_mat.shader = _scanline_shader
	shader_mat.set_shader_parameter("scanline_count", 80.0)
	...
	shader_mat.set_shader_parameter("tint_strength", 0.04)
	...
	_title_label.add_theme_font_override("font", _terminal_font)
	...
	_body_label.add_theme_font_override("font", _terminal_font)
...
func _slide_in() -> void:
	if _reduce_motion:
		# Skip animation — just place it at final position
	...
	var delay = DISPLAY_DURATION if _reduce_motion else DISPLAY_DURATION + SLIDE_DURATION
	_dismiss_timer = get_tree().create_timer(delay)
