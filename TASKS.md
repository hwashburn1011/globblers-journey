...
## CURRENT STATUS
- **Last updated by:** Claude (2026-04-05) — Task 15.8 complete
- **Last task completed:** Task 15.8 — NPC visual upgrades. Built 8 stylized NPC GLB models in Blender (man_page, sudo, batch_norm, sigmoid, gpt_classic, stable_diff, bert, sd_v1) with unique accessories (antenna, horns, visor, monocle, hat, backpack, bowtie, spikes) and chapter-themed accent colors. Updated `deprecated_npc.gd` to load GLB models via new `glb_path` export with fallback to legacy primitives. Wired all 8 NPCs across Ch1-4 level scripts. Source saved to `assets/blender_source/npc_models.blend`.
- **Next task to do:** Task 15.9 (Screen shake curve tuning)
