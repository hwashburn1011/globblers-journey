...
## CURRENT STATUS
- **Last updated by:** Claude (2026-04-05) — Task 15.7 complete
- **Last task completed:** Task 15.7 — Boss intro cinematic camera. Created `scripts/utils/boss_intro_camera.gd` (`BossIntroCamera` class_name) with static `play()` method: spawns temporary Camera3D, 3-second 270° orbital sweep around boss (radius/height scaled to collision capsule), slight zoom-in, then hands back to player camera via `make_current()`. Wired into all 5 bosses' `start_boss_fight()` — rm_rf, local_minimum, system_prompt, foundation_model, aligner. Respects reduce_motion (skips cinematic entirely). No runtime errors.
- **Next task to do:** Task 15.8 (NPC visual upgrades)
