...
## CURRENT STATUS
- **Last updated by:** Claude (2026-04-05) — Task 15.2 complete
- **Last task completed:** Task 15.2 — Floor & wall decals per chapter. Created 9 procedural decal textures in Blender (oil_puddle, scorch_mark, circuit_traces, circuit_emission, warning_stripes, runic_circle, ember_glow, dust_patch, light_pool) at 256x256 PNG with alpha. Built DecalPlacer utility (scripts/utils/decal_placer.gd) with static placement from room data. Added _place_decals() to all 5 chapters with themed configs: Ch1 green circuits + oil + scorch, Ch2 teal circuits + neural sigils, Ch3 ember glows + pink runic circles, Ch4 dust patches + faded stripes, Ch5 clinical light pools + alignment sigils. 5-6 decal types per chapter, ~25-30 Decal nodes per chapter.
- **Next task to do:** Task 15.3 (Environmental particles per chapter)
