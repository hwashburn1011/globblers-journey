...
## CURRENT STATUS
- **Last updated by:** Claude (2026-04-05) — Task 15.1 complete
- **Last task completed:** Task 15.1 — Globbler character animations. Rigged globbler.blend with 8-bone armature (Root, Spine, Head, Arm.L/R, WrenchHand, Leg.L/R), manually weight-painted 13502 verts, keyframed 5 animations (idle_bob 2s, walk 0.6s, run 0.4s, dash 0.25s, wrench_swing 0.4s), exported GLB with NLA strips. Updated globbler.gd with AnimationPlayer integration — skeleton clips play via state machine, procedural model_root overlays (bob/lean/squash) layered on top.
- **Next task to do:** Task 15.2 (Floor & wall decals per chapter)
